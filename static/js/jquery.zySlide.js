...
(function ($) {
    ...
    function Slide(ele, options) {
        ...
        this.states = [
            { '&zIndex': 1, width: 350, height: 365, top: 60, left: 0, $opacity: 0.6 },
            { '&zIndex': 2, width: 450, height: 465, top: 24, left: 110, $opacity: 0.7 },
            { '&zIndex': 3, width: 510, height: 555, top: 0, left: 300, $opacity: 1 },
            { '&zIndex': 2, width: 450, height: 465, top: 24, left: 620, $opacity: 0.7 },
            { '&zIndex': 1, width: 350, height: 365, top: 60, left: 840, $opacity: 0.6 }
        ]
        this.lis = this.$ele.find('li')
        this.interval
        // 点击切换到下一张
        this.$ele.find('li').on('mouseover', function () {
            this.stop()
        }.bind(this))
        this.$ele.find('li').on('mouseout', function () {
            this.play()
        }.bind(this))
        this.$ele.find('div.click_right').on('click', function () {
            this.stop()
            this.next()
            this.play()
        }.bind(this))
        // 点击切换到上一张
        this.$ele.find('div.click_left').on('click', function () {
            this.stop()
            this.prev()
            this.play()
        }.bind(this))
    ...
    Slide.prototype = {
        ...
        move: function () {
            ...
            this.lis.each(function (i, el) {
                $(el).css('z-index', this.states[i]['&zIndex']).animate(this.states[i], this.options.speed)
